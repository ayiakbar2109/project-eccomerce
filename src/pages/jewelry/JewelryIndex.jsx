...
import { setProductsJewelery } from "./../../redux/actions/productsAction";
    ...
    <section className="bg-white">
      <div className="mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className="grid grid-cols-2 gap-y-10 gap-x-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
          {getProductsJewelery.map((product) => (
            <a key={product.id} href={product.href} className="group">
              <div className="flex items-center justify-center h-4/6 w-full overflow-hidden rounded-lg xl:aspect-w-7 xl:aspect-h-8">
                <img
                  src={product.image}
                  alt={product.title}
